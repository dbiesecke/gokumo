...
post_makeinstall_target() {
  mkdir -p $INSTALL/etc/udevil
    ...
    if [ "$DISTRO" = "Gokumo" ]; then
      ...
      sed -i $INSTALL/etc/udevil/udevil.conf -e "s|^# mount_point_mode_curlftpfs =$|mount_point_mode_curlftpfs =|"
      sed -i $INSTALL/etc/udevil/udevil.conf -e "s|^# mount_point_mode_ftpfs =$|mount_point_mode_ftpfs =|"
    fi
